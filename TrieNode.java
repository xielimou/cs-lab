...
public class TrieNode {
    ...
    private boolean searchHelper(TrieNode n, String key){
        if (key.isEmpty()){
            return n.endsPath;
        }
        char firstChar = key.charAt(0);
    ...
    public void remove(String key) {
     ...
     private static void removeHelper(TrieNode n, String key) {
        ...
        if (child == null) {
            TrieNode newNode = new TrieNode();
            n.setChild(firstChar, newNode);
            removeHelper(newNode, key.substring(1));
    ...
    private static void matchHelper(TrieNode n, String key, String pattern) {
        if (n == null)
            return; 

        if (n.getEndsPath() && pattern.length()==0) {
            System.out.println(key);
        }
        if(pattern.length()==0)
            return;
        ...
        if (f == '?'){
            for (int j = 0; j < n.children.length; j++) {
                ...
                TrieNode child = n.children[j];
                char ch = indexToChar(j);
                matchHelper(child, key + ch, pattern.substring(1));
            }
            return;
        }
        
        else if (f == '*'){
            printKeysHelper(n, key);
            return;}
        

        else;{
            matchHelper(n.getChild(f), key+f, pattern.substring(1));}
        }
